namespace ServerWatchTower.Agent.ViewModel
{
    ...
    public partial class ServerEditViewModel
    {
        ...
        protected override async Task OnLoadAsync()
        {
            ...
            {
                this.Content.SortDescriptions.Add(new SortDescription("GUID", ListSortDirection.Ascending));
            ...
            if (this.OpenArgs != null)
            {
                // loading the server to be selected initially
                if (this.OpenArgs.SelectedServerGUID != null)
                {
                    var server = await this.Catalog.LoadItemAsync(this.serverCollection, this.OpenArgs.SelectedServerGUID);

                    if (this.IsDisposed) { return; }

                    if (this.ContentFilter(server))
                    {
                        if (this.Content.MoveCurrentTo(server))
                        {
                            this.BringCollectionItemIntoView(nameof(this.Content), server);
                        }
                    }
                }
            }
        ...
        /// </summary>
        private void ScheduleServerLoading()
        {
            ...
            if (!string.IsNullOrEmpty(this.QuickFilter))
            {
                ...
                {
                    args.QuickSearchWithinPartnerName = args.QuickSearchWithinServer = true;
                }
                else if (this.QuickFilter[0] == '^')
                {
                    args.QuickSearchWithinPartnerName = args.QuickSearchWithinServer = false;
                }
            }
        ...
        private void ExecuteOpenItem()
        {
            ...
            else
            {
                ...
                {
                    SelectedServerGUID = server.GUID
                };
                ...
                view.Closed +=
                    async (sender, e) =>
                    {
                        ...
                        {
                            var newServer = await this.Catalog.ReloadItemAsync(this.serverCollection, (string)e.ViewResult);

                            if (!this.IsDisposed && this.ContentFilter(newServer))
                            {
                                if (this.Content.MoveCurrentToKey(newServer.GUID))
                                {
        ...
        private bool ContentFilter(object item)
        {
            ...
            if (server.GUID.Contains(this.quickFilterCore, StringComparison.CurrentCultureIgnoreCase))
            {
                return true;
            }

            return server.Partner.HasWordStartingWith(this.quickFilterCore, StringComparison.CurrentCultureIgnoreCase);
        }
    }
}
